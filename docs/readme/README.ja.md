...
| アカウント | なし | ローカルアカウント |
| OIDC シングルサインオン | なし | 汎用 Provider、既存アカウント連携、任意のメンバー自動作成 |
...
- **原文に根ざした AI ネイティブ読書（Server・EPUB 専用）:** Server モードでは、章ガイド、根拠箇所とつながる解説、マインドマップ、振り返りの問い、非公開の Ask AI 対話を EPUB 原文のそばに置き、切り離された一般的な要約にしません。
- **プライベートな読書インサイト（Server 専用）:** 実際に読んだ時間、アクティビティカレンダー、傾向、セッション、最もよく読んだ本を確認できます。すべてのインサイトは、現在ログインしているアカウントだけに表示されます。

![EPUB 原文のそばに表示された章ガイドと、非公開の Ask AI パネル。](assets/ai-native-reading.png)

*AI ガイドと非公開の質問は、原文とのつながりを保ちます。*

![アクティビティカレンダーと読書時間の傾向を示す、非公開の読書インサイト画面。](assets/reading-insights.png)

*読書インサイトは、実際に読んだ時間を自分だけの理解しやすい履歴に変えます。*
