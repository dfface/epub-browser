...
class GeneratedReaderSurfaceTests(unittest.TestCase):
    ...
    def test_dictionary_import_uses_custom_file_controls_without_a_progress_surface(self):
        ...
        self.assertRegex(server_html, r'<input\b(?=[^>]*name=(?:["\'])?mdict_archive)(?=[^>]*class=(?:["\'])?dictionary-file-input)')
        self.assertRegex(server_html, r'\bdata-dictionary-file-name=(?:["\'])?mdict_archive(?:["\' >])')
        self.assertRegex(
            server_html,
            r'\baccept=(?:["\'])?\.zip,application/zip,application/octet-stream(?:["\' >])',
        )
        ...
        self.assertNotIn("dictionary_media_upload_failed", auth_js)
        self.assertIn("'X-EPUB-Browser-Dictionary-Format': format", auth_js)
...
'''
    ...
    def test_reader_overlays_close_outside_and_dictionary_picker_stays_compact(self):
        ...
        self.assertIn("dialog.className = 'annotation-dialog annotation-detail-dialog';", annotation)
        self.assertIn('getAnnotationAnchorRect: function(annotation)', annotation)
        self.assertIn('self.positionFloatingDialog(dialog, dialog._epubAnchor);', annotation)
        ...
        self.assertIn("var isPackagedHtml = format === 'mdict' || format === 'stardict:h';", dictionary)
        self.assertIn("definition.sandbox = allowScripts && (isPackagedHtml || isStarDictParts)", dictionary)
        self.assertIn("'allow-same-origin allow-scripts' : 'allow-same-origin';", dictionary)
        self.assertIn("default-src \\'none\\'; base-uri ' + origin + '; connect-src \\'none\\';", dictionary)
        self.assertIn("var scriptSource = allowScripts && (isPackagedHtml || isStarDictParts) ? origin + \" 'unsafe-inline'\" : \"'none'\";", dictionary)
        self.assertIn("script-src ' + scriptSource + '; style-src ' + origin + ' \\'unsafe-inline\\';", dictionary)
        self.assertIn("font-src ' + origin + ' data:", dictionary)
        self.assertIn('function resolveDictionaryPackageResources(source, entry, bookId, assetBasePath)', dictionary)
        self.assertIn('function dictionaryAssetBaseUrl(bookId, dictionaryId, assetBasePath)', dictionary)
        self.assertIn('function dictionaryAssetUrl(bookId, dictionaryId, assetBasePath, reference)', dictionary)
        self.assertIn("function renderStarDictParts(source, entry, bookId, assetBasePath)", dictionary)
        self.assertIn("format === 'stardict:parts'", dictionary)
